...
app = FastAPI(title="concepts-service", description="AI agent service", version="1.0.0")
...
async def startup_event():
    logger.info("concepts-service service starting up...")
    # Initialize any required resources
    pass

@app.on_event('shutdown')
async def shutdown_event():
    logger.info("concepts-service service shutting down...")
    client.close()
...
async def readiness_check():
    ...
    try:
        ...
        return {"status": "ready", "service": "concepts-service"}
...
async def chat_endpoint(request: ChatRequest, background_tasks: BackgroundTasks):
    """
    Main chat endpoint for the concepts agent
    """
    try:
...
async def process_agent_request(request: ChatRequest, session_state: Dict[str, Any]) -> str:
    ...
    return agent_responses.get("concepts", f"concepts agent received: {request.message}")
