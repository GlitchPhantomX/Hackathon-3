...
app = FastAPI(title="triage-service", description="AI agent service", version="1.0.0")
...
async def startup_event():
    logger.info("triage-service service starting up...")
    # Initialize any required resources
    pass

@app.on_event('shutdown')
async def shutdown_event():
    logger.info("triage-service service shutting down...")
    client.close()
...
async def readiness_check():
    ...
    try:
        ...
        return {"status": "ready", "service": "triage-service"}
...
async def chat_endpoint(request: ChatRequest, background_tasks: BackgroundTasks):
    """
    Main chat endpoint for the triage agent
    """
    try:
...
async def process_agent_request(request: ChatRequest, session_state: Dict[str, Any]) -> str:
    ...
    return agent_responses.get("triage", f"triage agent received: {request.message}")
